let StartFunc = async ({ inFetchResonse }) => {
    ...
    if (Object.values(jVarLocalResponseData).length > 0) {
        ...
        jFLocalToInputBranchName({ inInputFactorySelectedId: jVarLocalFetchData.BookingData.OrderData.BranchName })
        jFLocalRunSaveClick();
    };
};
...
let jFLocalToInputBranchName = ({inInputFactorySelectedId}) => {

    let jVarLocalHtmlId = 'InputBranchNameId';
    let jVarLocalBranchName = document.getElementById(jVarLocalHtmlId);

    if (inInputFactorySelectedId === null === false) {
        jVarLocalBranchName.value = inInputFactorySelectedId;
    };
};
