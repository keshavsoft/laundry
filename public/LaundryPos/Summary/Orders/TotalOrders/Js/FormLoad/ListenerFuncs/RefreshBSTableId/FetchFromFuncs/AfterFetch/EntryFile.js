...
let StartFunc = ({ inDataToShow }) => {
    ...
    let LocalFilterData = LocalDataToShow.filter(e => e.fileName == jVarLocalBranchName);

    let LocalmodifiedData = jFLocalShowDateDiffInMinSec({ inData: LocalFilterData });
   
    if ((LocalFilterData.length > 0) === false) swal.fire({ title: "No data !", icon: "error" });
...
let jFLocalShowDateDiffInMinSec = ({ inData }) => {
    ...
    jVarLocalReturnArray = inData.map(element => {
        ...
        if ((element.lastOrder === "") === false) {
            ...
            element.FirstOrderInterVal = jFLocalKInterval({ inCurrentdateandtime: element.firstOrder.OrderData.Currentdateandtime });
            element.OrderDate = element.lastOrder.OrderData.Currentdateandtime;
            element.FirstOrderpk = element.firstOrder.pk;
            element.LastOrderpk = element.lastOrder.pk;
            return element;
        };
        return element;
    });
